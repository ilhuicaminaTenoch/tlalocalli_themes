...
if (!function_exists('daiquiri_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'daiquiri_hovers_theme_setup3', 3 );
	function daiquiri_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		daiquiri_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'daiquiri'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'daiquiri') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'daiquiri'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'daiquiri'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'daiquiri') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'daiquiri'),
					'icon'	=> esc_html__('Icon',	'daiquiri')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('daiquiri_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'daiquiri_hovers_theme_setup9', 9 );
	function daiquiri_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'daiquiri_hovers_frontend_scripts', 1010 );
		add_filter( 'daiquiri_filter_localize_script','daiquiri_hovers_localize_script' );
		add_filter( 'daiquiri_filter_merge_scripts',	'daiquiri_hovers_merge_scripts' );
		add_filter( 'daiquiri_filter_merge_styles',	'daiquiri_hovers_merge_styles' );
		add_filter( 'daiquiri_filter_merge_styles_responsive', 'daiquiri_hovers_merge_styles_responsive' );
		add_filter( 'daiquiri_filter_get_css', 		'daiquiri_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'daiquiri_hovers_frontend_scripts' ) ) {
	
	function daiquiri_hovers_frontend_scripts() {
		if ( daiquiri_is_on(daiquiri_get_theme_option('debug_mode')) && daiquiri_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'daiquiri-hovers', daiquiri_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('daiquiri_hovers_merge_styles')) {
	
	function daiquiri_hovers_merge_styles($list) {
...
if (!function_exists('daiquiri_hovers_merge_styles_responsive')) {
	
	function daiquiri_hovers_merge_styles_responsive($list) {
...
if (!function_exists('daiquiri_hovers_localize_script')) {
	
	function daiquiri_hovers_localize_script($arr) {
		$arr['button_hover'] = daiquiri_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('daiquiri_hovers_merge_scripts')) {
	
	function daiquiri_hovers_merge_scripts($list) {
...
if ( !function_exists('daiquiri_hovers_add_icons') ) {
	function daiquiri_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					daiquiri_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'daiquiri') : esc_html__('Buy now', 'daiquiri')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'daiquiri');
						else
							esc_html_e('Details', 'daiquiri');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-link-1"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'daiquiri_hovers_get_css' ) ) {
	
	function daiquiri_hovers_get_css($css, $colors, $fonts, $scheme='') {
