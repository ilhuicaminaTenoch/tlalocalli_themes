...
?>><?php
	...
	// Featured image
	if ( daiquiri_is_off(daiquiri_get_theme_option('hide_featured_on_single'))
			&& !daiquiri_sc_layouts_showed('featured') 
	...
	// Title and post meta
	if ( (!daiquiri_sc_layouts_showed('title') || !daiquiri_sc_layouts_showed('postmeta')) && !in_array(get_post_format(), array('link', 'aside', 'status', 'quote')) ) {
		do_action('daiquiri_action_before_post_title'); 
		?>
		<div class="post_header entry-header">
			<?php
			// Post title
			if (!daiquiri_sc_layouts_showed('title')) {
				the_title( '<h3 class="post_title entry-title"'.($daiquiri_seo ? ' itemprop="headline"' : '').'>', '</h3>' );
			}
			// Post meta
			if (!daiquiri_sc_layouts_showed('postmeta') && daiquiri_is_on(daiquiri_get_theme_option('show_post_meta'))) {
		...
		<?php
		do_action('daiquiri_action_after_post_title'); 
	}
	...
	<div class="post_content entry-content" itemprop="mainEntityOfPage">
		...
		do_action('daiquiri_action_before_post_pagination'); 
		...
		if ( is_single() && !is_attachment() ) {
			...
			?><div class="post_meta post_meta_single"><?php
				...
				// Post taxonomies
				the_tags( '<span class="post_meta_item post_tags"><span class="post_meta_label">'.esc_html__('Tags:', 'daiquiri').'</span> ', ' ', '</span>' );

				// Share
				if (daiquiri_is_on(daiquiri_get_theme_option('show_share_links'))) {
	...
	if ( daiquiri_get_theme_option('show_author_info')==1 && is_single() && !is_attachment() && get_the_author_meta( 'description' ) ) {
		do_action('daiquiri_action_before_post_author'); 
		get_template_part( 'templates/author-bio' );
		do_action('daiquiri_action_after_post_author'); 
	}
