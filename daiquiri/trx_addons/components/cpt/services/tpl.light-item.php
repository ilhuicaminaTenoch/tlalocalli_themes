...
<div class="sc_services_item<?php
	...
	echo esc_attr($args['featured']=='image'
					? ' with_image' 
					: ($args['featured']=='icon' 
						? ' with_icon' 
	...
	echo esc_attr($video ? ' hover_video' : '');
?>"<?php
	if (!empty($args['popup'])) {
...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
															...
															'thumb_size' => apply_filters('trx_addons_filter_thumb_size', daiquiri_get_thumb_size((int) $args['columns'] > 1 ? 'big-extra' : 'full'), 'services-light'),
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			...
			?>
			<<?php echo esc_attr($title_tag); ?> class="sc_services_item_title<?php if (!$price_showed && !empty($meta['price'])) echo ' with_price'; ?>"><a href="<?php echo esc_url($link); ?>"><?php
				the_title();
				// Price
