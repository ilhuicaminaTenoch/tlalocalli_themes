...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['prices'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('big'), 'price-default'));
				if (!empty($item['image'])) {
					$attr = trx_addons_getimagesize($item['image']);
					?><div class="sc_price_item_image" style="<?php echo 'background-image:url('.esc_url($item['image']).');'; ?>">
			...
			?><div class="sc_price_item_info"><?php
				...
				if (empty($item['image']) && !empty($item['subtitle'])) {
					$item['subtitle'] = explode('|', $item['subtitle']);
					?><h6 class="sc_price_item_subtitle"><?php
						foreach ($item['subtitle'] as $str) {
							?><span><?php echo esc_html($str); ?></span><?php
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'daiquiri_kses_content'); ?></span><?php
						}
					?></div><?php
				}
					...
					?><div class="sc_price_item_details"><?php trx_addons_show_layout($item['details']); ?></div><?php
						...
						if (!empty($item['price'])) {
							...
							if (!empty($item['before_price'])) {
								?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'daiquiri_kses_content'); ?></span><?php
							}
							?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'daiquiri_kses_content'); ?></span><?php
							if (count($parts) > 1 && $parts[1]!='') {
								?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'daiquiri_kses_content'); ?></span><?php
							}
							if (!empty($item['after_price'])) {
								?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'daiquiri_kses_content'); ?></span><?php
							}
							?></div><?php
						}
