...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) ); ?>><?php
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'hover' => 'zoomin',
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', daiquiri_get_thumb_size((int) $args['columns'] > 1 ? 'big-extra' : 'full'), 'blogger-default')
														), 'blogger-default')
								);
	...
	?><div class="sc_blogger_item_content entry-content"><?php
		...
		if (!isset($args['hide_excerpt']) || (int) $args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				...
				// More button
				if ( $show_more ) {
					?><div class="sc_blogger_item_button sc_item_button"><a href="<?php echo esc_url($post_link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button color_style_default sc_button_default sc_button_size_normal', 'sc_blogger', $args)); ?>"><?php esc_html_e('Read more', 'daiquiri'); ?></a></div><?php
				}
